...
const Onboarding = () => {
  ...
  const handleStepChange = (step) => {
    // Track if user provided input before moving to next step
  ...
  const handleComplete = async () => {
    ...
    // Update user profile with all collected preferences
    if (likedItems.length > 0 || allGenres.size > 0) {
      try {
        await updateUser({
          ...
          preferences: {
            ...user.preferences,
            favorite_genres: Array.from(allGenres),
  ...
  // Determine if we need Step 4 (Genre Selection)
